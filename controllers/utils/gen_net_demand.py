...
@author: Allen Woods
@modify: Alvin Lee
@time: 2020.6.10 21:29
@file: gen_net_demand.py
@desc:
'''
...
def add_sumo_path():
    try:
        assert os.environ.get("SUMO_HOME") is not None

    except AssertionError:
        sys.exit(
            "please declare environment variable 'SUMO_HOME' "
            "as the root directory of your sumo installation (it "
            "should contain folders 'bin', 'tools' and 'docs')"
            )#
            #

    sumotools_dir = os.path.join(os.environ.get("SUMO_HOME"), 'tools')
    sys.path.insert(0, sumotools_dir)
...
class SumoCfg:
    def __init__(self, data_dir, net_name, xnumber, ynumber,
                 xlength=1000, ylength=1000, lane_num=2, net_type='grid', tls_type='static', rouprob=10,
        ...
        self.detectors = [os.path.join(self.net_dir, self.netname + '_e%d.add.xml' % (i + 1))
                          for i in range(3)]
        self.sumocfg = os.path.join(self.net_dir, self.netname + '.sumocfg')
    ...
    def init(self, withdet=False):
        ...
        :return:
        """
        self.gen_network(self.xnumber, self.ynumber, self.xlength, self.ylength,self.lane_num,
    ...
    def gen_network(self, xnumber, ynumber, xlength, ylength,lane_num,
        ...
        :param xlength: length of each edge on x axis
        :param ylength: length of each edge on y axis
        :param lane_num: number of lanes on each edge
    ...
    def gen_randomtrips(self, rouprob, endtime, period=None, binomial=None,
        ...
        rantrip_generator = os.path.join(SUMO_HOME,'tools', 'randomTrips.py')
...
if __name__ == '__main__':
    ...
    grid_num_x = input('Input the columns number of grid roadnet please:')
    grid_num_y = input('Input the rows number of grid roadnet please:')
    generate_net_demand(data_dir,grid_num_x,grid_num_y)